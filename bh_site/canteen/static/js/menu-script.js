Vue.component('category', {
  props: ['image', 'name', 'num_dishes', 'id'],
  template: `
        ...
        <v-hover>
            <v-card slot-scope="{ hover }" class="mx-auto" color="grey lighten-4" max-width="600">
              <v-img :src="image" height="200px">
                <v-expand-transition>
                  <a v-if="hover" @click="go_to('/product', id)" class="d-flex transition-fast-in-fast-out orange v-card--reveal display-3 white--text mb1"   style="height: 100%;"></a>
                </v-expand-transition>
                <v-card-title primary-title>
                  <div style="background: rgba(0, 0, 0, 0.4); padding: 15px; border-radius:5px;">
                    <div class="headline" style="text-shadow: 2px 2px 5px black">{{ name }}</div>
                    <span class="white--text" style="text-shadow: 2px 2px 5px black">{{ num_dishes }} блюд</span>
                  </div>
                </v-card-title>
              </v-img>
            </v-card>
...
Vue.component('menu-app', {
  ...
  data: () => ({
    ...
    category: [
      // {
      //   "image": "https://www.gastronom.ru/binfiles/images/20161019/b4e889a6.jpg",
      //   "name": "Первые блюда",
      //   "num_dishes": "8",
      // },
    ]
  ...
  created: function(){
    // var self = this;
    // category_load_handler = function(data){
    //   console.log(data)
    //   // for (i in data){
    ...
    // get_categories(self.$store.state.canteen_id, category_load_handler)
  },
  computed: {
    category_list: async function (){
      ...
      // alert(self.$store.state.canteen_id)
      await get_categories(self.$store.state.canteen_id, category_load_handler)
